...
io.on("connection", (socket) => {
  ...
  socket.emit("message", "Welcome! I'm happy to assist you today.");
...
const handleMessage = (socket, message) => {
  ...
  switch (true) {
    ...
    case messageStr.startsWith("/flip"):
      ...
      case messageStr.startsWith("Hi"):
        socket.emit("message", "Hey there! How can I help you?");
        break;  
        
        case messageStr.startsWith("What is Flutter"):
          socket.emit("message", "Flutter is an open-source framework created by Google for building beautiful, native-compiled applications for mobile, web, desktop, and embedded devices from a single codebase......");
          break;  



        case messageStr.startsWith("What is the day today"):
          socket.emit("message", "It's Friday");
          break;  
